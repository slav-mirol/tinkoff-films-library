...
const App = (props) => {
    ...
    useEffect(() => {
      fetch('http://localhost:3300/movies').then((res) => res.json())
    ...
    async function fetchAll() {
      await fetch('http://localhost:3300/movies').then((res) => res.json())
      .then((data) => setMovies(data))
      .catch(() => alert('Error in fetch!'));
    }
    ...
    const main_content = chooseMovie ? <MovieDetails movie={curMovie} setChooseMovie={setChooseMovie} editMovie={callEditMovie}/> : 
                     isAddMovie ? <AddMovie addMovie={addMovie} setIsAddMovie={setIsAddMovie} /> : 
                     isEditMovie ? <AddMovie movie={editedMovie} setEditMovie={setEditMovie} editMovie={editMovie} showMovie={showMovie}/>  : <></>;
    return (
      <div>
        <Header/>
        <div className='main-content'>
          <MovieList allFilms={allFilms} showMovie={showMovie} callAddMovie={callAddMovie} movies={movies} setMovies={setMovies} isEditMovie={isEditMovie} />
          {main_content}
        </div>
      </div>
    );
